...
def create_job():
    ...
    st.caption("※現在、感情分析は英語投稿のみに対応しています。日本語投稿はneutralと判定される可能性があります。")
    ...
    if st.button("取得開始"):
        ...
        if not reddit_client_id or not reddit_client_secret or not reddit_user_agent:
            st.error("Reddit API認証情報が設定されていません。.envファイルを確認してください。")
            error_flag = True
    ...
    if st.session_state.get("job_started", False):
        ...
        try:
            ...
            if not posts:
                ...
                st.info("※指定キーワードに関連する投稿が期間内に見つからなかったか、最大取得数（100件）に含まれていなかった可能性があります。")
            else:
                semantic_filtered_posts = semantic_search(posts, params["keyword"], top_k=100)
                start_ts = datetime.datetime.combine(params["start_date"], datetime.time.min).timestamp()
                end_ts = datetime.datetime.combine(params["end_date"], datetime.time.max).timestamp()
                date_filtered_posts = [p for p in semantic_filtered_posts if start_ts <= p['created_utc'] <= end_ts]
                ...
                if not date_filtered_posts:
                    ...
                    sentiment_results = analyze_sentiment(date_filtered_posts, lang="auto")
                    st.session_state["last_results"] = {
    ...
    if last_results and "sentiment_results" in last_results:
        ...
        if output_format == "棒グラフ":
            ...
            positive_texts = [res["text"] for res in sentiment_results if res["sentiment"] == "positive"]
            negative_texts = [res["text"] for res in sentiment_results if res["sentiment"] == "negative"]
            col1, col2 = st.columns(2)
            with col1:
        ...
        for res in sentiment_results:
            st.markdown(f"- {res['text'][:60]}... ({res['sentiment']}) [出所]({res['url']})")
